...
import { SystemPromptFactory } from "@/runtime/context/SystemPromptFactory"
...
export async function runAgent(
  ...
  const systemPrompt = SystemPromptFactory.build({
    role: normalizedRole,
    executionMode,
    projectRules,
  })
  const contextResult = await ContextAssembler.getInstance().assemble(userMessage, config.role)
