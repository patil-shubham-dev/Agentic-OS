...
import { RuntimeInspector } from "@/runtime/runtime-inspector"
...
import { ExecutionDebugPanel } from "@/components/workspace/execution-debug-panel"
import { ExecutionExplorer } from "@/components/runtime/ExecutionExplorer"
import { RightPanel } from "@/components/runtime/RightPanel"
import { RuntimeStatusBar } from "@/components/runtime/RuntimeStatusBar"
import { ValidationCenter } from "@/components/runtime/ValidationCenter"
import { ExecutionDock } from "@/components/runtime/ExecutionDock"
import { ErrorBoundary } from "@/components/runtime/ErrorBoundary"
import { HydrationGate } from "@/components/runtime/HydrationGate"
...
import {
  ...
  Users, Brain, Activity, CheckCircle2, XCircle, AlertTriangle,
  ChevronDown, Palette,
  GripVertical, History, Bug,
} from "lucide-react"
...
export function CodeCanvasPage() {
  ...
  const { hasIssues, hasErrors, report, issuesByType, lastRepairResult, runRepair, runValidation } = useIntegrity()
  const navigate = useNavigate()
  const [showDiagnostics, setShowDiagnostics] = useState(false)
  const [diagnosticsTab, setDiagnosticsTab] = useState<"errors" | "warnings" | "info">("errors")
  ...
  // ── Validation state from preflight + integrity ──
  ...
  useEffect(() => {
    ...
    if (hasErrors) {
      const errs = issuesByType?.errors ?? []
      for (const err of errs.slice(0, 10)) {
        issues.push({
          id: `val-err-${issues.length}`,
          severity: "error",
          category: "configuration",
          message: (err as any).message ?? String(err),
          detail: (err as any).suggestion ?? "",
          repairable: !!(err as any).repairable,
          repairAction: (err as any).repairable ? "repair-config" : undefined,
        })
      }
    }
  ...
  }, [report, issuesByType, hasErrors, runtimeReady, runtimeStatus, hasStaleConfig, runtimeMessage])

  const handleValidationDismiss = useCallback((id: string) => {
    setValidationIssues((prev) => prev.filter((i) => i.id !== id))
  }, [])

  const handleValidationRepair = useCallback((issue: ValidationIssue) => {
  ...
  useEffect(() => {
    function handleKey(e: KeyboardEvent) {
      ...
      if ((e.metaKey || e.ctrlKey) && e.shiftKey && e.key === "r") {
        e.preventDefault()
        setShowRuntimeInspector((p) => !p)
      }
      if ((e.metaKey || e.ctrlKey) && e.shiftKey && e.key === "d") {
        e.preventDefault()
        setShowExecutionDebug((p) => !p)
      }
  ...
  const handleRightPanelResize = useCallback(() => {
    const resizeRef = { current: true }
    document.body.style.cursor = "col-resize"
    document.body.style.userSelect = "none"

    function onMouseMove(e: MouseEvent) {
      if (!resizeRef.current) return
      const newWidth = Math.max(280, Math.min(600, window.innerWidth - e.clientX - 52))
  ...
  return (
    <div className="flex h-full flex-col bg-gradient-to-b from-[#0a0a0b] to-[#09090a]" role="main" aria-label="Code canvas workspace">
      {showRuntimeInspector && (
        <ErrorBoundary name="RuntimeInspector">
          <div className="border-b border-yellow-500/15 bg-yellow-500/[0.03] px-4 py-3">
            <RuntimeInspector />
          </div>
        </ErrorBoundary>
      )}
      ...
      {hasIssues && runtimeReady && (
        <>
          <button
            onClick={() => setShowDiagnostics(!showDiagnostics)}
            className={cn(
              "flex items-center gap-2 border-b px-4 py-2 w-full text-left transition-all hover:bg-white/[0.02]",
              hasErrors ? "border-red-500/15 bg-red-500/[0.03]" : "border-amber-500/15 bg-amber-500/[0.03]",
            )}
          >
            <AlertTriangle className={cn("h-3 w-3 shrink-0", hasErrors ? "text-red-400" : "text-amber-400")} />
            <span className={cn("text-xs", hasErrors ? "text-red-400" : "text-amber-400")}>
              {hasErrors
                ? `${report?.summary.errors ?? 0} integrity error(s) found`
                : `${report?.summary.warnings ?? 0} integrity warning(s) found`}
              {(lastRepairResult?.repairsSucceeded ?? 0) > 0 && (
                <> — {(lastRepairResult?.repairsSucceeded ?? 0)} issue(s) auto-fixed</>
              )}
            </span>
            <div className="ml-auto flex items-center gap-1">
              <span className="text-[9px] text-white/30">
                {(() => {
                  const fixableCount = (issuesByType?.errors ?? []).filter(i => i.repairable).length +
                    (issuesByType?.warnings ?? []).filter(i => i.repairable).length
                  const manualCount = (hasErrors ? (issuesByType?.errors ?? []).length : 0) +
                    (issuesByType?.warnings ?? []).length - fixableCount
                  return (
                    <>
                      {fixableCount > 0 && <span className="text-green-400/60">{fixableCount} auto-fixable</span>}
                      {manualCount > 0 && <span className="text-amber-400/60"> • {manualCount} need attention</span>}
                    </>
                  )
                })()}
              </span>              <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation()
                          // First run auto-reconcile
                          runRepair()
                          // Then navigate to settings for remaining manual fixes
                          navigate('/settings')
                        }}
                      >
              ...
              <ChevronDown className={cn("h-3 w-3 text-white/30 transition-transform", showDiagnostics && "rotate-180")} />
            </div>
          </button>
          {showDiagnostics && (
            <div className="border-b border-white/[0.06] bg-white/[0.015] px-4 py-3 max-h-48 overflow-y-auto">
              <div className="flex gap-2 mb-2">
                {(["errors", "warnings", "info"] as const).map((t) => {
                  const count = issuesByType?.[t]?.length ?? 0
                  return (
                    <button
                      key={t}
                      onClick={() => setDiagnosticsTab(t)}
                      className={cn(
              ...
              <div className="space-y-1">
                {(issuesByType?.[diagnosticsTab] ?? []).map((issue: any, idx: number) => (
                  <div key={idx} className="text-[11px] text-white/50 flex items-start gap-2 py-0.5">
                    <span className={cn(
                      "shrink-0 mt-0.5 h-1.5 w-1.5 rounded-full",
                      issue.severity === "error" ? "bg-red-500" : issue.severity === "warn" ? "bg-amber-500" : "bg-blue-500"
                    )} />
                    <span>{issue.message}</span>
                    {issue.repairable && (
                      <span className="text-[9px] text-green-500/60 shrink-0 ml-auto">auto-fixable</span>
                    )}
                  </div>
                ))}
                {issuesByType[diagnosticsTab].length === 0 && (
                  <p className="text-[11px] text-white/30">No {diagnosticsTab} found.</p>
                )}
              </div>
            </div>
          )}
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0 min-h-0" role="region" aria-label="Assistant chat panel">
        ...
        <div className="flex items-center justify-between px-3 py-1.5 border-b border-white/[0.06] bg-[#0c0c0d]">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => setShowRuntimeInspector(!showRuntimeInspector)}
                className={cn(
                  ...
                  runtimeReady && runtimeHealth === "healthy"
                    ? "border-green-500/15 bg-green-500/[0.04] text-green-400 hover:bg-green-500/[0.08]"
                    : runtimeReady && runtimeHealth === "degraded"
                      ? "border-amber-500/15 bg-amber-500/[0.04] text-amber-400 hover:bg-amber-500/[0.08]"
              ...
              <button
                ...
                aria-label={rightPanelOpen ? "Close timeline panel" : "Open timeline panel"}
              >
        ...
        {/* Resize handle: Assistant | Workspace Panel */}
        {workspacePanelOpen && <ResizeHandle onMouseDown={handleWorkspaceResize} />}

        {/* PANEL 3: Dynamic Workspace Panel */}
        <div
        ...
        >
          {/* Workspace panel tabs — polished toggle row */}
          <div className="flex items-center bg-[#0c0c0d] border-b border-white/[0.04] px-1.5" role="tablist" aria-label="Workspace panels">
        ...
        {/* PANEL 4: Right Panel (Timeline / Tools / Reasoning / Terminal) */}
        {rightPanelOpen && <ResizeHandle onMouseDown={handleRightPanelResize} />}
        <div
          style={{ width: rightPanelOpen ? rightPanelWidth : 0 }}
          className={cn(
            "flex-shrink-0 flex flex-col overflow-hidden min-h-0",
            rightPanelOpen && "border-l border-white/[0.06]",
          )}
        >
      ...
      {/* Execution Debug Panel — collapsible overlay drawer */}
      <AnimatePresence>
        {showExecutionDebug && (
          <motion.div
            ...
            className="border-t border-blue-500/15 bg-blue-500/[0.015] overflow-hidden"
          >
            <div className="px-4 py-3 max-h-[400px] overflow-y-auto">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] font-medium text-blue-400">Execution Debug</span>
                <button
                  onClick={() => setShowExecutionDebug(false)}
      ...
      <div className="flex items-stretch border-t border-white/[0.02]">
        ...
        <ValidationCenter
          ...
          onRepair={handleValidationRepair}
          onDismiss={handleValidationDismiss}
        />
        <button
          onClick={() => setShowExecutionDebug(!showExecutionDebug)}
          className={cn(
            "flex items-center gap-1 px-2 text-[9px] border-l border-white/[0.02] transition-all",
            showExecutionDebug
              ? "bg-blue-500/10 text-blue-400"
              : "text-white/20 hover:text-white/50 hover:bg-white/[0.03]",
          )}
          title="Toggle execution debug panel (⌘⇧D)"
        >
