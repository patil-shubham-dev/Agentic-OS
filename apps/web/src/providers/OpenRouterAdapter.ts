...
export class OpenRouterAdapter extends BaseProviderAdapter {
  ...
  constructor(apiKey: string, baseUrl?: string) {
    super("openrouter", "OpenRouter", baseUrl ?? "https://openrouter.ai/api/v1", apiKey)
  }
        ...
        for (const m of (data as any).data) {
          models.push({
            ...
            capabilities: {
              streaming: true,
              toolCalls: true,
              vision: !!m.architecture?.modality?.includes("vision"),
              reasoning: false,
              maxContextWindow: m.context_length ?? 8192,
  ...
  async stream(request: CompletionRequest, callbacks: StreamCallbacks, signal?: AbortSignal): Promise<void> {
    ...
    ;(headers as Record<string, string>)["HTTP-Referer"] = "https://agentic-os.studio"
    ;(headers as Record<string, string>)["X-Title"] = "Agentic-OS Studio"
