...
import { ContextAssembler } from "@/runtime/context/ContextAssembler"
...
export class ExecutionOrchestrator {
  ...
  private contextAssembler = ContextAssembler.getInstance()
  ...
  async buildAgentContext(input: string, role: string): Promise<string> {
    const context = await this.contextAssembler.assemble(input, role)
  ...
  private async handleDirectResponse(
    ...
    if (wiredForFastChat) {
      ...
      try {
        const result = await fastChatCompletion(fcProvider.baseUrl, fcProvider.apiKey, wiredForFastChat.model, input, history, ctrl.signal)
      ...
      const groupPromises = parallelGroup.map(async (taskId) => {
        ...
        try {
          ...
          const roleSpecificInput = [
            input,
            await this.contextAssembler.assemble(input, runtimeRole).then((c) => c.promptBlock),
          ].filter(Boolean).join("\n\n")
