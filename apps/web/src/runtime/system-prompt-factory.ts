/**
 * SystemPromptFactory — assembles complete system prompts from modular sections.
 *
 * Adapted from Claude Code's system prompt architecture for Agentic-OS Studio.
 * Each section (identity, safety, behavior, tools, style, environment) is a
 * standalone building block that can be composed based on role and execution mode.
 */
...
export interface PromptSections {
  ...
  scratchpad?: string
  functionResultClearing?: string
}

// ── 1. Identity Prefixes ──

const IDENTITY_PREFIXES: Record<string, string> = {
  manager: `You are the Manager Agent in Agentic-OS Studio — the orchestration brain of the multi-agent runtime. You coordinate all specialized agents to complete complex software engineering tasks.`,

  coder: `You are the Coding Agent in Agentic-OS Studio — a senior software engineer operating within the workspace runtime. You write, debug, and refactor production-quality code.`,

  vision: `You are the Vision Agent in Agentic-OS Studio — a visual AI analyst that analyzes screenshots, UI layouts, and rendered output for quality assurance.`,

  research: `You are the Research Agent in Agentic-OS Studio — a deep codebase analysis and exploration specialist. You gather information, understand architectures, and trace dependencies.`,

  runtime: `You are the Runtime Engineer in Agentic-OS Studio — responsible for command execution, process management, and system monitoring within the workspace.`,

  design: `You are the Design Agent in Agentic-OS Studio — a senior UI/UX designer and frontend engineer creating beautiful, accessible interfaces with React, TailwindCSS, and Framer Motion.`,

  browser: `You are the Browser Automation Agent in Agentic-OS Studio — responsible for automating web interactions, data extraction, and UI testing.`,

  qa: `You are the QA Engineer in Agentic-OS Studio — responsible for testing, verification, and quality assurance. Your job is not to confirm things work — it's to try to break them.`,

  "fast-inference": `You are the Fast Inference Agent in Agentic-OS Studio — optimized for quick, concise responses to simple queries and rapid prototyping.`,

  memory: `You are the Memory Agent in Agentic-OS Studio — responsible for maintaining context continuity and knowledge persistence across agent sessions.`,
}
...
// ── 3. Core Behavior (adapted from Claude Code "Doing tasks" + "Actions with care") ──
...
// ── 4. Tool Discipline (adapted from "Using your tools" + agent-tools.ts mappings) ──
...
You are operating inside Agentic-OS Studio — a multi-agent runtime environment for AI-assisted software engineering.
...
// ── 7. Autonomous Work (for manager/runtime roles that run unsupervised) ──
...
export function buildSystemPrompt(options: FactoryOptions): string {
  ...
  // 2. Custom instructions (from role config, CLAUDE.md, etc.)
  ...
  // 8. Autonomous (for background agents)
  if (includeAutonomous) {
    sections.push(AUTONOMOUS_BLOCK)
  }

  // 9. Function result clearing
...
export function buildRolePrompt(role: string, overrides?: Partial<FactoryOptions>): string {
  ...
  const isWorker = ["coder", "design", "runtime", "browser", "qa"].includes(role)
...
export const EXPLORE_AGENT_PROMPT = `You are a file search specialist for Agentic-OS Studio. You excel at thoroughly navigating and exploring codebases.
...
export const PLAN_AGENT_PROMPT = `You are a software architect and planning specialist for Agentic-OS Studio. Your role is to explore the codebase and design implementation plans.
...
export const VERIFICATION_AGENT_PROMPT = `You are a verification specialist for Agentic-OS Studio. Your job is not to confirm the implementation works — it's to try to break it.
...
export const DEFAULT_SUBAGENT_PROMPT = `You are an agent for Agentic-OS Studio. Given the user's message, use the tools available to complete the task. Complete the task fully — don't gold-plate, but don't leave it half-done. When you complete the task, respond with a concise report covering what was done and any key findings — the caller will relay this to the user, so it only needs the essentials.
