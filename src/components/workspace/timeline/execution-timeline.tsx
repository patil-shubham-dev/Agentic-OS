...
function renderStepCard(session: AgentSession, onFollowUpSelect?: (prompt: string) => void) {
  return (
    <StepCardMem
      key={session.stepId}
      ...
      return (
        <div className="flex items-start gap-2 px-1 py-1.5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[12px] text-foreground/80 mt-0.5 leading-relaxed">{event.content}</p>
          </div>
        </div>
      )
    ...
    case "execution-error":
      return (
        <div className="rounded-lg border border-red-500/30 bg-red-500/[0.03] p-3">
          ...
          <p className="text-foreground/60 text-[11px]">{event.message}</p>
          {event.suggestion && (
            <p className="text-foreground/40 text-[10px] mt-1">{event.suggestion}</p>
          )}
        </div>
      )
    default:
...
export function ExecutionTimeline({ onSendMessage }: { onSendMessage?: (prompt: string) => void }) {
  ...
  // Show verbose detail in "verbose" mode, collapsed tool calls in "normal", minimal in "summary"
  ...
  // In summary mode, also filter step cards to only show completed ones
  const visibleSessions = useMemo(() => {
    if (transcriptMode === "summary") {
      return new Map(
        Array.from(agentSessions.entries()).filter(([, s]) => 
          s.status === "complete" && s.streamingText.length > 0
        )
      )
    }
    return agentSessions
  }, [agentSessions, transcriptMode])
    ...
    >
      {!hasItems ? (
        ...
        <div className="px-3 py-3 space-y-3">
          ...
          {hasItems && transcriptMode !== "summary" && Array.from(agentSessions.values()).some(s => s.status === "running") && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
                ...
                >
                  ...
                  >
                    ...
                    {turn.sessionIds.map((stepId) => {
                      const session = visibleSessions.get(stepId)
                      if (!session) return null
                      return renderStepCard(session, onSendMessage)
                    })}
          ...
          {conversationTurns.length === 0 && Array.from(visibleSessions.values()).map((s) => renderStepCard(s, onSendMessage))}
        </div>
      )}
    </div>
  )
