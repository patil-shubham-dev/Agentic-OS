...
// ── Inline Tool Execution (uses shared implementations from tool-executor) ──
 ...
 * Uses the shared implementations from the tool executor.
 */
async function executeSubAgentTools(toolCalls: ToolCall[]): Promise<ChatMessage[]> {
  ...
  const results = await Promise.allSettled(
    toolCalls.map(async (tc): Promise<InlineToolResult> => {
      ...
      try {
        await sandbox.assertAllowed({ id: tc.id, name: tc.function.name, args }, { role: "sub-agent" })

        let data: string
        switch (tc.function.name) {
          case "grep_files":
            data = await implGrepFiles(rootPath, String(args.pattern ?? ''), args.include as string | undefined)
            break
          case "glob_files":
            data = await implGlobFiles(rootPath, String(args.pattern ?? ''))
            break
          case "read_file":
            data = await implReadFile(rootPath, String(args.path ?? ''))
            break
          case "write_file":
            data = await implWriteFile(rootPath, String(args.path ?? ''), String(args.content ?? ''))
            break
          case "edit_file":
            data = await implEditFile(rootPath, args as any)
            break
          case "run_command":
            data = await implRunCommand(rootPath, "sub-agent", tc.id, String(args.command ?? ''), args.args as string[] | undefined)
            break
          default:
            data = `Unknown tool: ${tc.function.name}`
        }

        return { tool_call_id: tc.id, role: "tool", content: data }
      } catch (e) {
        return { tool_call_id: tc.id, role: "tool", content: `Error executing ${tc.function.name}: ${String(e)}` }
      }
    }),
  )
